...
import { HiOutlineUserAdd, HiOutlineCalendar, HiOutlineClock, HiOutlinePlus } from 'react-icons/hi';
import { LuScissors } from 'react-icons/lu';

interface StaffsEmptyProps {
    onCreateClick: () => void;
}

export default function StaffsEmpty({ onCreateClick }: StaffsEmptyProps) {
    return (
        <div className="relative max-w-3xl mx-auto my-6 bg-white rounded-2xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] overflow-hidden p-8 md:p-12 text-center flex flex-col items-center">
            ...
            <div className="relative mb-8 mt-4 select-none">
                ...
                <div className="relative flex items-center justify-center w-24 h-24 rounded-full bg-linear-to-br from-primary/10 to-blue-500/5 border border-primary/20 text-primary shadow-xs">
                    <HiOutlineUserAdd size={36} className="text-primary/90" />

                    {/* Ring Pulse Effect */}
                    <div className="absolute inset-0 rounded-full border-2 border-primary/20 animate-ping opacity-60 pointer-events-none" />
                </div>

                {/* Floating Badge 1: Calendar (Top Left) */}
                <div className="absolute -top-3 -left-8 bg-emerald-50 text-emerald-600 border border-emerald-100/50 p-2.5 rounded-xl shadow-[0_4px_12px_rgba(16,185,129,0.08)] flex items-center justify-center rotate-[-10deg] hover:rotate-0 hover:scale-110 transition-all duration-300">
                    <HiOutlineCalendar size={20} />
                </div>

                {/* Floating Badge 2: Scissors (Bottom Right) */}
                ...
                {/* Floating Badge 3: Clock (Top Right) */}
                <div className="absolute -top-4 -right-6 bg-amber-50 text-amber-600 border border-amber-100/50 p-2 rounded-xl shadow-[0_4px_10px_rgba(245,158,11,0.08)] flex items-center justify-center rotate-[8deg] hover:rotate-0 hover:scale-110 transition-all duration-300">
            ...
            <p className="text-sm md:text-base text-gray-500 max-w-lg mx-auto mb-8 leading-relaxed">
                Start by adding your first stylist. You&apos;ll be able to manage their schedules,
                assign services, and set up individual booking availability all in one place.
            </p>

            {/* Action Button */}
            <Button
                id="add-first-staff-btn"
